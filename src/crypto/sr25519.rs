...
impl Sr25519 {
    ...
    pub fn from_suri(suri: &str) -> DomainResult<Pair> {
        Pair::from_string(suri, None)
            .map_err(|e| DomainError::CryptoError(format!("Sr25519 from SURI failed: {:?}", e)))
    }
...
mod tests {
    ...
    fn test_ss58_address() {
        let pair = Sr25519::from_suri(TEST_SEED).unwrap();
        let public = Sr25519::public_key(&pair);
        let ss58 = Sr25519::to_ss58_address(&public);

        // Known test vector from README
        assert_eq!(ss58, "5DfhGyQdFobKM8NsWvEeAKk5EQQgYe9AydgJ7rMB6E1EqRzV");
    }

    #[test]
    fn test_derivation_path() {
        let suri_with_path = format!("{}//midnight//governance//0", TEST_SEED);
        let pair = Sr25519::from_suri(&suri_with_path).unwrap();
        let public = Sr25519::public_key(&pair);

        // Should be different from root key
        let root_pair = Sr25519::from_suri(TEST_SEED).unwrap();
        let root_public = Sr25519::public_key(&root_pair);
    ...
    fn test_sign_verify() {
        ...
        let signature = Sr25519::sign(&pair, message);
        let public = Sr25519::public_key(&pair);

        assert!(Sr25519::verify(&public, message, &signature));
        assert!(!Sr25519::verify(&public, b"wrong message", &signature));
    }

    #[test]
    fn test_to_key_material() {
        let pair = Sr25519::from_suri(TEST_SEED).unwrap();
        let key_material = Sr25519::to_key_material(
            &pair,
            KeyPurpose::Governance,
            Some("//midnight//governance//0".to_string()),
        );

        assert_eq!(key_material.key_type, KeyTypeId::Sr25519);
        assert_eq!(key_material.purpose, KeyPurpose::Governance);
